...
const Home = () => {
  ...
  const onDragEnd = async (result) => {
    ...
    const { success, message } = await updateTaskListId(
      taskId,
      destinationListId
    );
    if (success) {
          ...
          <div className="w-full h-full flex items-start justify-start space-x-2 overflow-x-auto listbox">
            {!listLoading &&
              lists.map((list, idx) => (
                <div
                  key={list.id}
                  className="min-w-[250px] w-1/4 h-full border"
                >
                  ...
                  <Droppable droppableId={String(list.id)}>
                    {(provided) => (
                      <div
                        ...
                        className="w-full h-full flex flex-col items-start justify-start py-2 px-1"
                      >
                        {tasks
